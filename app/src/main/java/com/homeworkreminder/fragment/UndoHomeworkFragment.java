...
public class UndoHomeworkFragment extends Fragment {
    ...
    public void addRecyclerView(){
        ...
        //System.out.println(myRecyclerView);

        //设置RecyclerView的布局管理器
        myRecyclerView.setLayoutManager(new LinearLayoutManager(getActivity(), LinearLayout.VERTICAL,false));

        //初始化适配器
        undoDataRecyclerViewAdapter = new UndoDataRecyclerViewAdapter(getActivity(), homeDataList);

        //设置动画
        myRecyclerView.setItemAnimator(new DefaultItemAnimator());

        //设置适配器
        myRecyclerView.setAdapter(undoDataRecyclerViewAdapter);
